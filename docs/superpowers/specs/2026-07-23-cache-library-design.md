...
Accepts a **caller-owned** `redis.UniversalClient`, so a single node, a sentinel setup, a `*redis.ClusterClient` or a `*redis.Ring` all work; `Close` is a no-op. The two multi-key operations detect a sharded client (by the `ForEachMaster` / `ForEachShard` methods, masters first so a cluster never fans out to replicas) and route around two silent failures. `DeleteMany` cannot batch one `DEL` there — keys in different hash slots are rejected with `CROSSSLOT` on a cluster, and a Ring would route the whole command by its first key and silently leave the rest — so it pipelines one `DEL` per key and lets go-redis split the pipeline across nodes, keeping the same 500-key batches; the single batched `DEL` stays for unsharded clients. `ClearPrefix` cannot drive one `SCAN` cursor either: neither client offers a cluster-wide `SCAN` and go-redis picks a node per call, so the cursor walks an arbitrary node's keyspace and the clear reports success having missed every other master. It runs the whole cursor loop against every master (`ForEachMaster`) or shard (`ForEachShard`) instead — concurrently, with per-node state confined to the callback — and deletes through the sharded delete path. Prefix clearing on a Ring is best-effort by construction: `ForEachShard` skips a shard that is down.
...
- **Conformance suite** (`driver/drivertest`, imports `gocache`): exported functions asserting Get/Set/TTL/Add/DeleteIfEquals/ClearPrefix semantics, run by every driver. Default runs: memory directly, Redis via miniredis — twice, once on a single server and once through a `*redis.Ring` spread over three of them, which is the only unit-test coverage of the driver's sharded paths — and SQL via a file-backed temporary SQLite database (`:memory:` is per-connection in modernc.org/sqlite, so it cannot serve a multi-connection pool). The null driver cannot satisfy the suite — it stores nothing, so read-back and `Add` semantics do not apply — and has its own targeted test instead.
